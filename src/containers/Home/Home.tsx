...
class HomeComponent extends React.Component {
  ...
  async componentWillMount() {
    ...
    await this.setState({
      user
      // numberreps: user.profile_data.total_repos || 0,
      // numbersets: user.profile_data.total_sets || 0
    });
    this.setState({
  ...
  difficultlist = async () => {
    const token = await AsyncStorage.getItem("token");
    this.props.diffcultiListApi(token);
  };
  ...
  stepreps(item) {
    console.log("Step Reps: ", item);
    // console.log('Step Reps: ', this.state)
    AsyncStorage.setItem("totalreps", item.reps);
    AsyncStorage.setItem("totalsets", item.steps);
  }
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        >
          <Image
            resizeMode="contain"
            style={Platform.isPad ? styles.leftLogoiPad : styles.leftLogoiPhone}
          ...
          <View
            style={{
              ...
              alignItems: "center",
              marginTop: Platform.isPad ? 60 : 30
            }}
          >
          ...
          <View
            style={
              Platform.isPad ? styles.logoutViewiPad : styles.logoutViewiPhone
            }
          >
            <TouchableOpacity
              style={Platform.isPad ? styles.logoutiPad : styles.logoutiPhone}
        ...
        <ScrollView
          style={{
            backgroundColor: "transparent",
            marginTop: Platform.isPad ? 120 : 60
          }}
        >
          <View>
          ...
          >
            ...
            <View style={{ flexDirection: "row" }}>
              <Image
                resizeMode="contain"
                style={{ width: 20, height: 20, marginTop: 10, marginRight: 3 }}
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    diffcultiListApi: token =>
      dispatch(DifficultListActions.diffcultiListApi(token))
  };
};
